...
和最小确认。它不选择音符、不解释情感、不生成唱法名称，也不暗中修改 Agent
请求的音乐数值。

### Lua 执行器

Lua 层读取权威 SynthV 对象，使用当前宿主能力，展开确定性操作，校验每个
目标和结果值，并且只在完整预检之后到达 `Project:newUndoRecord()`。
如果同一次新鲜读取没有提供有效的 Automation `definition.range`，自动化
控制点写入会安全失败。执行器还会验证支持的写后条件。

### SynthV 和用户

SynthV 是工程状态权威。用户选择目标 Group 和歌手，提供接口无法读取的
唱法名称，试听预览，判断结果是否符合歌曲，并在需要时使用 SynthV 撤销。

## 批处理准入规则

只有当一个动作是机械的、确定性的、有界的、可预览的，并且能在一个撤销
记录之前完整验证时，才应加入批处理。`transform_notes` 符合这些条件，
