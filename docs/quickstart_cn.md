...
第一次让 AI 修改工程前，请先保存重要工作。在 SynthV 中选择一个较短的
乐句，例如 2～8 小节，先进行只读检查：
...
SynthV 脚本 API 无法读取当前歌手身份，也无法列出尚未启用的默认
Vocal Mode。对于从未进行过任何调整、仍然保持默认值的 Vocal Mode，官方
接口无法读取其名称和唱法参数。因此，返回空的 Vocal Mode 列表并不代表
当前歌手没有 Vocal Mode，而是 Bridge 无法通过官方接口发现这些仅包含
默认值的唱法。

第一次修改某个歌手的 Vocal Mode 前，请选择一种方式：

- 把面板中显示的所有 Vocal Mode 名称完整告诉 Codex，并保留原有拼写和
  大小写；或
- 提供一张包含完整 Vocal Mode 面板的截图。

完成首次识别后，只要没有更换歌手，Codex 就可以继续使用同一份唱法名称
列表。更换歌手后，需要重新提供列表或截图。
...
4. 如果是第一次调整当前歌手的 Vocal Mode，请发送完整 Vocal Mode 面板
   截图，或者准确输入面板中显示的全部唱法名称。
5. 先选择一段歌词，让 Agent 使用已经识别的 Vocal Mode 调整整段的总体
