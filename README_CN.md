...
- 读取工程元数据、完整速度/拍号图、播放状态、轨道、库 Group、音符、
  完整选区状态、计算音素/音高、Smart Pitch 对象、自动化曲线、编辑器
  视口和混音器状态。
- 创建、克隆、复用、更新或删除库音符 Group，以及人声/乐器 Group 引用。
- 克隆现有轨道以继承歌手/数据库，并可选择清空或移调克隆的音符。
- 添加音符，并编辑单音符的语言、歌唱/说唱类型、自动音高模式、说唱重音、
  时值、音高、歌词、音素、微调和属性。V2 会把通过轨道主 Group 插入的
  音符自动放入可复用的非主 Group，使 Group Voice 和 Vocal Mode 仍可编辑。
- 读取并安全更新类型化的 Group Voice 默认值、Vocal Mode
  音高/音色/发音轴，以及宿主返回的实验性 Unison 字段。空的 Vocal Mode
  映射可以使用用户提供的准确名称，在一次克隆验证的批处理中完成初始化，
  无需逐模式探测。
- 同时读取计算音素和用户音素，并安全编辑音素集覆盖、音节时值，以及
  单音素的时值/强度属性。
- 在陈旧写入保护下添加、编辑和删除点或曲线 Smart Pitch 控制。
- 生成、激活和删除由 Bridge 跟踪的 AI Retake。
- 使用最新音符指纹安全编辑或删除音符。
- 在秒、四分音符和 blick 之间转换，并编辑速度/拍号标记。
- 添加、替换、采样、简化或清除音高偏差、响度、张力、气声、发声、
  性别和 Vocal Mode 等自动化曲线。
- 读取和修改选区与视口状态，使用网格吸附和坐标转换，并通过 SynthV
  宿主剪贴板交换文本。
- 控制增益、声像、静音、独奏、播放、暂停、停止、定位和循环。
- 预检最多 32 个相互独立的写入，并在一个 SynthV 撤销记录中应用完整
  事务；可选支持当前会话内受保护的回滚。
- 创建受音域约束的和声轨、人性化音符时值、把歌词匹配到音符，并应用
  Scoop、Falloff、Vibrato、Crescendo 或 Breathiness 预设。
- 将每次成功的写入调用或事务放入一个 SynthV 撤销记录。
- 可选使用 SynthV 原生侧边栏检查连接/当前选区上下文、排队一条指令、
  审核结构化受保护写入或事务、应用/放弃/取消、查看诊断和近期活动。
...
- SynthV 脚本 API 不公开当前歌手身份，也无法枚举仍保持默认值、从未调整
  过的 Vocal Mode 名称和参数。修改 Vocal Mode 前，用户应按原始拼写和
  大小写列出当前歌手面板中的全部准确名称，或提供 Vocal Mode 面板截图。
- 同一歌手可复用已经识别的名称；更换歌手后才需要再次告知 Agent。

Agent 每次对话只需给出一次提示，不必在每次编辑前重复。用户可以提供
如下简短提示：

```text
当前歌手的 Vocal Modes：Airy、Bright、Cool、Dark、Emotional、Power、
Solid、Sweet。
```

也可以提供一张清楚显示完整 Vocal Mode 面板的截图。
